...
int sectfusion( ELF_STRUCT file1, ELF_STRUCT file2, ELF_STRUCT fileres, sect_tab * tab){
	...
	int j =0;
	int place=0;
	tab = malloc(sizeof(sect_tab)*(file1.header->e_shnum+file2.header->e_shnum));
	...
	for (int i=0; i<length2; i++) {
		...
		if (file2.shtab[i].sh_type == SHT_PROGBITS) {
			...
			if (!appartient) {
				...
				fileres.shtab[cpt].sh_name += file1.shtab[file1.header->e_shstrndx].sh_size;
				fileres.shtab[cpt].sh_offset=place;
