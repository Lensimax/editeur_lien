...
int symbolfus(ELF_STRUCT file_1, ELF_STRUCT file_2, sect_tab * sect_fus, ELF_STRUCT * file_final, int nb_sect_after_fusion, int * resultat_nb_symboles){ /////// malloc du symtab_final à faire dans le main (malloc de 1)
	...
	unsigned char* fileBytes_1 = readFileBytes(file_1.file);
	unsigned char* fileBytes_2 = readFileBytes(file_2.file;
...
///////////////////////////////////PARCOURS DE LA 1ERE TABLE DE SYMBOLES
	while(!ELF32_ST_BIND(file_1.symtab[p1].st_info) && p1 < (file_1.shtab[indice_symtab_1].sh_size/file_1.shtab[indice_symtab_1].sh_entsize)) {
	...
	while(!ELF32_ST_BIND(file_2.symtab[p2].st_info) && p2 < (file_2.shtab[indice_symtab_2].sh_size/file_2.shtab[indice_symtab_2].sh_entsize)) {
	...
	for (int k=p1;k<file_1.shtab[indice_symtab_1].sh_size/file_1.shtab[indice_symtab_1].sh_entsize;k++) {
		boolean = 0;
			...
			addrStrName_1_var = addrStrName_1_stable+file_1.symtab[k].st_name;
			j = 0;
				//recuperation du symbole
				while(fileBytes_1[addrStrName_1_var] != '\0'){
				nameString_1[j] = fileBytes_1[addrStrName_1_var];
				addrStrName_1_var++;
				j++;
				}
			...
			for (int l=0;l<file_2.shtab[indice_symtab_2].sh_size/file_2.shtab[indice_symtab_2].sh_entsize;l++) {
				...
				if(ELF32_ST_BIND(file_2.symtab[l].st_info) != 0) {

				nameString_2 = malloc(sizeof(char)*75);
				//position en debut de nom du symbole
				addrStrName_2_var = addrStrName_2_stable+file_2.symtab[l].st_name;
				j = 0;
					//recuperation du symbole
					while(fileBytes_2[addrStrName_2_var] != '\0'){
					nameString_2[j] = fileBytes_2[addrStrName_2_var];
					addrStrName_2_var++;
					j++;
					}
				//ajout de la marque de fin
				...
				if(strcmp(nameString_1,nameString_2) == 0) {
					boolean = 1;
					if(file_2.symtab[l].st_shndx != SHN_UNDEF && file_1.symtab[k].st_shndx != SHN_UNDEF) {
						printf("ERREUR 2 SYMBOLES DEFINIS DE MEME NOMS");
						return -1;

					} else if((file_2.symtab[l].st_shndx == SHN_UNDEF && file_1.symtab[k].st_shndx != SHN_UNDEF) || (file_1.symtab[k].st_shndx == SHN_UNDEF && file_2.symtab[l].st_shndx == SHN_UNDEF)) {
...
///////////////////////////////////PARCOURS DE LA 2EME TABLE DE SYMBOLES
	...
	for (int k=p2;k<file_2.shtab[indice_symtab_2].sh_size/file_2.shtab[indice_symtab_2].sh_entsize;k++) {
		boolean = 0;
			...
			nameString_2 = malloc(sizeof(char)*75);
			//position en debut de nom du symbole
			addrStrName_2_var = addrStrName_2_stable+file_2.symtab[k].st_name;
			j = 0;
				//recuperation du symbole
				while(fileBytes_2[addrStrName_2_var] != '\0'){
				nameString_2[j] = fileBytes_2[addrStrName_2_var];
			...
			for (int l=0;l<file_1.shtab[indice_symtab_1].sh_size/file_1.shtab[indice_symtab_1].sh_entsize;l++) {
				...
				if(ELF32_ST_BIND(file_1.symtab[l].st_info) != 0) {

				nameString_1 = malloc(sizeof(char)*75);
				//position en debut de nom du symbole
				addrStrName_1_var = addrStrName_1_stable+file_1.symtab[l].st_name;
				j = 0;
					//recuperation du symbole
					while(fileBytes_1[addrStrName_1_var] != '\0'){
					nameString_1[j] = fileBytes_1[addrStrName_1_var];
				...
				if(strcmp(nameString_2,nameString_1) == 0) {
					boolean = 1;
					if(file_1.[l].st_shndx != SHN_UNDEF && file_2.symtab[k].st_shndx != SHN_UNDEF) {
						printf("ERREUR 2 SYMBOLES DEFINIS DE MEME NOMS");
						return -1;
					} else if(file_1.[l].st_shndx == SHN_UNDEF && file_2.symtab[k].st_shndx != SHN_UNDEF) { // *var_1.st_shndx == SHN_UNDEF && *var_2.st_shndx != SHN_UNDEF
